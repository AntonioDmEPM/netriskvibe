...
const metrics: Metric[] = [
  { icon: Users, label: "Households onboarded", target: "50,000" },
  { icon: Wallet, label: "Avg savings per household", target: "£500+" },
  { icon: TrendingUp, label: "Revenue per household", target: "£250+" },
        ...
        {metrics.map((m, i) => (
          <ScrollReveal key={i} delay={i * 60}>
            <div className="rounded-xl border border-border bg-card p-5 text-center transition-transform duration-150 hover:scale-[1.03] active:scale-[0.98] cursor-default h-full">
              ...
              <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1.5">{m.label}</p>
