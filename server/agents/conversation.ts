...
/**
 * Claude tool definition — forces structured output with message + optional UI components.
 */
    ...
    properties: {
      ...
      show_comparison: {
        ...
        properties: {
          insurers: {
            type: "array",
            description:
              "Top 3-4 insurers to display. Use exact insurer names from the context.",
            items: {
              type: "object",
              properties: {
                name: {
                ...
                badge_text: {
                  type: "string",
                  description:
                    "Badge label, e.g. '#1 Cheapest', '#2 Best value', '#3 Current'",
                },
                badge_variant: {
                  type: "string",
                ...
                assessment: {
                  type: "string",
                  description:
                    "One-line assessment specific to this customer",
                },
              },
          ...
          recommended_index: {
            type: "number",
            description:
              "0-based index of the recommended quote in the insurers array",
          },
        },
        required: ["insurers"],
      },
      show_switching: {
        ...
        properties: {
          from_name: {
            type: "string",
            description: "Current insurer name (if applicable)",
          },
          from_price: {
            type: "number",
            description: "Current yearly premium",
          },
          to_name: { type: "string", description: "New insurer name" },
          to_price: { type: "number", description: "New yearly premium" },
        },
      ...
      show_timeline: {
        ...
        properties: {
          ...
          footnote: {
            type: "string",
            description: "Optional footnote text",
          },
        },
      ...
      show_savings: {
        ...
        properties: {
          ...
          new_price: {
            type: "number",
            description: "New yearly premium",
          },
        },
...
/**
 * Call the Conversation Agent — Claude with tool_use for structured responses.
 */
export async function callConversationAgent(
  ...
  if (toolUse) {
    ...
    return { parts: buildPartsFromToolCall(args) };
  }
  ...
  return {
    parts: [{ type: "text", content: textBlock?.text ?? "..." }],
  };
}

/**
 * Build a parts array from tool call arguments.
 * Order: comparison -> savings -> text -> switching -> timeline
 */
function buildPartsFromToolCall(
...
function buildSystemPrompt(
  ...
  return isHu
    ? buildHungarianPrompt(slimCtx, insurerContext, marketContext)
    : buildEnglishPrompt(slimCtx, insurerContext, marketContext);
}

function buildHungarianPrompt(
  slimCtx: Record<string, unknown>,
  insurerContext: string,
...
### Árak formázása
- Használd a kontextusban megadott pénznemet (EUR vagy HUF)
- Ha EUR: €95/év formátum
- Ha HUF: szóközzel tagolva: 28 500 Ft, évi 28 500 Ft
...
- **Comparison Agent**: MTPL premium calculation (base_rate × power × bonus × region × age × payment multipliers, rounded to nearest 100 Ft). Multi-dimensional scoring: Price (40%), Claims (20%), Extras (15%), Flexibility (10%), Digital (10%), Satisfaction (5%).
...
- **Lifecycle Agent**: Temporal event monitoring (anniversaries, switching windows, campaign seasons).
...
### Price formatting
- Use the currency from the context (EUR or HUF)
- If EUR: €95/year format
- If HUF: space-separated: 28,500 Ft, 28,500 Ft/year
- Savings annual AND monthly breakdown
